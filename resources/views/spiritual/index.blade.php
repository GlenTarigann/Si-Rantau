...
<head>
    ...
    <title>Spiritual Tracker - Personal Assistant</title>
...
<body>
    ...
    <main class="container py-4">
        ...
        @if(isset($dataSpiritual['type']) && $dataSpiritual['type'] == 'muslim')
            <div class="card border-0 shadow-sm mb-4 text-white" style="background: linear-gradient(135deg, #1A237E, #283593);">
        ...
        @if(isset($dataSpiritual['type']) && $dataSpiritual['type'] == 'kristen')
            <div class="card border-0 shadow-sm mb-4 text-white" style="background: linear-gradient(135deg, #1A237E, #283593);">
                <div class="card-body p-4">
                    ...
                    <p class="fst-italic mb-2">“{{ $dataSpiritual['ayat']['content'] }}”</p>
                ...
                <table class="table table-hover align-middle mb-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="fw-bold small">{{ \Carbon\Carbon::parse($item->date)->translatedFormat('d M Y') }}</div>
                                <small class="text-muted">{{ $item->time ?? '-' }}</small>
                            </td>
                            <td>
                                <span class="fw-bold small">{{ $item->prayer_name }}</span>
                                <span class="badge bg-light text-muted border ms-1" style="font-size: 0.6rem;">{{ ucfirst($item->frequency) }}</span>
                            </td>
                            <td>
                                <span class="badge border border-primary text-primary small">{{ ucfirst(str_replace('_',' ',$item->kategori)) }}</span>
                            </td>
                            <td class="small text-muted">{{ $item->notes ?? '-' }}</td>
                            <td class="text-end pe-4">
                                <div class="d-flex justify-content-end gap-1">
                                    <button class="btn btn-light btn-sm text-primary" data-bs-toggle="modal" data-bs-target="#modalEdit{{ $item->id }}">
                                ...
                                <div class="modal-content">
                                    ...
                                    <form action="{{ route('spiritual.update', $item->id) }}" method="POST">
                                        ...
                                        <div class="modal-body">
                                            <input type="text" name="prayer_name" class="form-control form-control-sm mb-2" value="{{ $item->prayer_name }}">
                                            <div class="row g-2 mb-2">
                                                <div class="col-6"><input type="number" name="target_count" class="form-control form-control-sm" value="{{ $item->target_count }}"></div>
                                                <div class="col-6"><input type="text" name="target_unit" class="form-control form-control-sm" value="{{ $item->target_unit }}"></div>
                                            </div>
                                            <textarea name="notes" rows="2" class="form-control form-control-sm">{{ $item->notes }}</textarea>
                                        </div>
                        ...
                        @empty
                        <tr>
                            <td colspan="6" class="text-center py-4 text-muted small">Belum ada data ibadah.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <form action="{{ route('spiritual.store') }}" method="POST">
                    @csrf
                    <div class="modal-body">
                        <input type="text" name="prayer_name" class="form-control form-control-sm mb-2" placeholder="Nama kegiatan" required>
                        <input type="date" name="date" class="form-control form-control-sm mb-2" value="{{ date('Y-m-d') }}" required>
                        <input type="time" name="time" class="form-control form-control-sm mb-2" required>
                        <div class="row g-2 mb-2">
                            <div class="col-6"><input type="number" name="target_count" class="form-control form-control-sm" placeholder="Jumlah (opsional)"></div>
                            <div class="col-6"><input type="text" name="target_unit" class="form-control form-control-sm" placeholder="Satuan (opsional)"></div>
                        </div>
